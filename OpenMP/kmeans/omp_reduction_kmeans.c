			...
			double * clusters)         /* out: [numClusters][numCoords] */
{
	...
	{
		local_newClusterSize[k] = (typeof(*local_newClusterSize)) calloc(numClusters + PADDING_SIZE, sizeof(**local_newClusterSize));
		local_newClusters[k] = (typeof(*local_newClusters)) calloc((numClusters * numCoords) + PADDING_SIZE, sizeof(**local_newClusters));
	}
	...
	do {
        ...
        #pragma omp parallel
        {
            ...
            for (i=0; i<numClusters; i++) {
                for (j=0; j<numCoords; j++)
                    local_newClusters[thread_num][i*numCoords + j + PADDING_SIZE] = 0.0;
                local_newClusterSize[thread_num][i + PADDING_SIZE] = 0;
            }   
		
            #pragma omp for private(i, j, index) reduction(+:delta) schedule(static)
            for (i=0; i<numObjs; i++)
            {
                 ...
                 */
                local_newClusterSize[thread_num][index + PADDING_SIZE]++;
                for (j=0; j<numCoords; j++)
                    local_newClusters[thread_num][index*numCoords + j + PADDING_SIZE] += objects[i*numCoords + j];
            }
            ...
            for (k=0; k<nthreads; k++) {
                for (i=0; i<numClusters; i++) {
                    newClusterSize[i] += local_newClusterSize[k][i + PADDING_SIZE];
                    for (j=0; j<numCoords; j++)
                        newClusters[i*numCoords + j] += local_newClusters[k][i*numCoords + j + PADDING_SIZE];
                }
            }
        }
