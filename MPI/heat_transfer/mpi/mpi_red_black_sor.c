...
int main(int argc, char ** argv) {
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        if (north != MPI_PROC_NULL) MPI_Sendrecv(&u_previous[1][1], 1, row_type, north, 0, &u_previous[0][1], 
                                    1, row_type, north, 0, MPI_COMM_WORLD, &status);

        // communicate with south
        if (south != MPI_PROC_NULL) MPI_Sendrecv(&u_previous[local[0]][1], 1, row_type, south, 0, &u_previous[local[0]+1][1], 
                                    1, row_type, south, 0, MPI_COMM_WORLD, &status);

        // communicate with east
        if (east != MPI_PROC_NULL) MPI_Sendrecv(&u_previous[1][local[1]], 1, col_type, east, 0, &u_previous[1][local[1]+1],
                                   1, col_type, east, 0, MPI_COMM_WORLD, &status);

        // communicate with west
        if (west != MPI_PROC_NULL) MPI_Sendrecv(&u_previous[1][1], 1, col_type, west, 0, &u_previous[1][0],
        ...
        // communicate with north
        if (north != MPI_PROC_NULL) MPI_Sendrecv(&u_current[1][1], 1, row_type, north, 0, &u_current[0][1],
                                    1, row_type, north, 0, MPI_COMM_WORLD, &status);

        // communicate with south
        if (south != MPI_PROC_NULL) MPI_Sendrecv(&u_current[local[0]][1], 1, row_type, south, 0, &u_current[local[0]+1][1],
                                    1, row_type, south, 0, MPI_COMM_WORLD, &status);

        // communicate with east
        if (east != MPI_PROC_NULL) MPI_Sendrecv(&u_current[1][local[1]], 1, col_type, east, 0, &u_current[1][local[1]+1],
                                   1, col_type, east, 0, MPI_COMM_WORLD, &status);

        // communicate with west
        if (west != MPI_PROC_NULL) MPI_Sendrecv(&u_current[1][1], 1, col_type, west, 0, &u_current[1][0],
                                   1, col_type, west, 0, MPI_COMM_WORLD, &status);
